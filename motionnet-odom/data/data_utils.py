...
def voxelize_occupy_gpu(pts, voxel_size, extents=None, return_indices=False):
    """
    Voxelize the input point cloud. We only record if a given voxel is occupied or not, which is just binary indicator.
    ...
    if extents is not None:
        ...
        filter_idx = cp.where((extents[0, 0] < pts[:, 0]) & (pts[:, 0] < extents[0, 1]) &
                              (extents[1, 0] < pts[:, 1]) & (pts[:, 1] < extents[1, 1]) &
                              (extents[2, 0] < pts[:, 2]) & (pts[:, 2] < extents[2, 1]))[0]
        pts = pts[filter_idx]

    # Discretize voxel coordinates to given quantization size
    discrete_pts = cp.floor(pts[:, :3] / voxel_size).astype(cp.int32)

    # Use Lex Sort, sort by x, then y, then z
    x_col = discrete_pts[:, 0]
    y_col = discrete_pts[:, 1]
    z_col = discrete_pts[:, 2]
    sorted_order = cp.lexsort(cp.asarray([z_col, y_col, x_col]))

    # Save original points in sorted order
    discrete_pts = cp.asnumpy( discrete_pts[sorted_order] )
    ...
    voxel_coords = cp.asarray(discrete_pts[unique_indices])

    # Compute the minimum and maximum voxel coordinates
    if extents is not None:
        min_voxel_coord = cp.floor(extents.T[0] / voxel_size)
        max_voxel_coord = cp.ceil(extents.T[1] / voxel_size) - 1
    else:
        min_voxel_coord = cp.amin(voxel_coords, axis=0)
        max_voxel_coord = cp.amax(voxel_coords, axis=0)

    # Get the voxel grid dimensions
    num_divisions = ((max_voxel_coord - min_voxel_coord) + 1).astype(cp.int32)

    # Bring the min voxel to the origin
    voxel_indices = (voxel_coords - min_voxel_coord).astype(int)

    # Create Voxel Object with -1 as empty/occluded
    leaf_layout = VOXEL_EMPTY * cp.ones(cp.asnumpy(num_divisions).astype(np.int), dtype=cp.float32)
...
def gen_non_empty_map_gpu(refer_pc: cp.array, grid_size: cp.array, extents: cp.array = None):
    """
    ...
    # ----------------------------------------------------
    # Filter and sort the reference point cloud
    assert refer_pc.shape[1] <= 5, "Input pointcloud should be of shape (N,3)"
    refer_pc = refer_pc[:, 0:3]
    if extents is not None:
        if extents.shape != (3, 2):
            raise ValueError("Extents are the wrong shape {}".format(extents.shape))

        filter_idx = cp.where((extents[0, 0] < refer_pc[:, 0]) & (refer_pc[:, 0] < extents[0, 1]) &
                              (extents[1, 0] < refer_pc[:, 1]) & (refer_pc[:, 1] < extents[1, 1]) &
                              (extents[2, 0] < refer_pc[:, 2]) & (refer_pc[:, 2] < extents[2, 1]))[0]
        refer_pc = refer_pc[filter_idx]
    # -- Discretize pixel coordinates to given quantization size
    discrete_pts = cp.floor(refer_pc[:, 0:2] / grid_size).astype(cp.int32)
    # -- Use Lex Sort, sort by x, then y
    x_col = discrete_pts[:, 0]
    y_col = discrete_pts[:, 1]
    sorted_order = cp.lexsort(cp.asarray([y_col, x_col]))
    discrete_pts = cp.asnumpy(discrete_pts[sorted_order])
    contiguous_array = np.ascontiguousarray(discrete_pts).view(
        np.dtype((np.void, discrete_pts.dtype.itemsize * discrete_pts.shape[1])))
    # -- The new coordinates are the discretized array with its unique indexes
    ...
    # -- Sort unique indices to preserve order
    unique_indices.sort()
    pixel_coords = cp.asarray(discrete_pts[unique_indices])
    # -- Compute the minimum and maximum voxel coordinates
    if extents is not None:
        min_pixel_coord = cp.floor(extents.T[0, 0:2] / grid_size)
        max_pixel_coord = cp.ceil(extents.T[1, 0:2] / grid_size) - 1
    else:
        min_pixel_coord = cp.amin(pixel_coords, axis=0)
        max_pixel_coord = cp.amax(pixel_coords, axis=0)
    # -- Get the voxel grid dimensions
    num_divisions = ((max_pixel_coord - min_pixel_coord) + 1).astype(cp.int32)
    # -- Bring the min voxel to the origin
    pixel_indices = (pixel_coords - min_pixel_coord).astype(int)
    # ----------------------------------------------------
    # Set the non-zero pixels to 1.0
    # Note that the non-zero pixels correspond to the foreground and background objects
    non_empty_map = cp.zeros([int(num_divisions[0]), int(num_divisions[1])], dtype=cp.float32)
    non_empty_map[pixel_indices[:, 0], pixel_indices[:, 1]] = 1.0
