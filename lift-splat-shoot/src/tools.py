...
def get_val_info(model, valloader, loss_fn, device, use_tqdm=False):
    ...
    total_intersect = 0.0
    total_union = 0
    total_intersect_static = 0.0
    total_union_static = 0
    total_intersect_dynamic = 0.0
    total_union_dynamic = 0
    print('running eval...')
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # iou static
            intersect_static, union_static, _ = get_batch_iou(preds[:, 0, :, :], binimgs[:, 0, :, :])
            total_intersect_static += intersect_static
            total_union_static += union_static

            # iou dynamic
            intersect_dynamic, union_dynamic, _ = get_batch_iou(preds[:, 1, :, :], binimgs[:, 1, :, :])
            total_intersect_dynamic += intersect_dynamic
            total_union_dynamic += union_dynamic

    model.train()
    return {
            'loss': total_loss / len(valloader.dataset),
            'iou': total_intersect / total_union,
            'iou_static': total_intersect_static / total_union_static,
            'iou_dynamic': total_intersect_dynamic / total_union_dynamic,
            }
